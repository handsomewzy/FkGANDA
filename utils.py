...
def build_dataset(mat, gt, ignored_labels=None):
    ...
    #         # 1*200*7*7 的图片。取出中间的通道
    #         samples += list(img[0][:][3][3])
    #         labels += [label]
...
def metrics(prediction, target, ignored_labels=[], n_classes=None):
    ...
    for i in range(len(accuracy_scores)-1):
        B = B + accuracy_scores[i+1]
    else:
        aa = B / (len(accuracy_scores)-1)
    results["AA"] = aa*100
...
def show_results(results, vis, label_values=None, agregated=False):
    ...
    if agregated:
        ...
        accuracies_every = [r["Accuracy_scores"] for r in results]
        accuracies_every_mean = np.mean(accuracies_every, axis=0)
        accuracies_every_std = np.std(accuracies_every, axis=0)
    ...
    text += "每一类的精确度如下："
    if agregated:
        for label, score, std in zip(label_values, accuracies_every_mean,
                                     accuracies_every_std):
            text += "\t{}: {:.03f} +- {:.03f}\n".format(label, score, std)
    else:
