...
if __name__ == "__main__":
    for i in [8,10,15,16]:
        ...
        # Optimizers
        optimizer_G = torch.optim.RMSprop(generator.parameters(), lr=0.0005)
        optimizer_D = torch.optim.RMSprop(discriminator.parameters(), lr=0.0005)
        ...
        for epoch in range(opt.n_epochs):
            ...
            # 模型的存储
            if opt.save_model:
                torch.save(generator.state_dict(), "WGAN_%d_%s_%f.pth" % (opt.choose_labels[0], opt.dataset_name,
                                                                         opt.training_sample))
