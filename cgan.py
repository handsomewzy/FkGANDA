...
parser.add_argument("--choose_labels", default=[1, 3, 4, 7, 9, 12, 15], help="the target we choose to generate")
...
# Loss functions
adversarial_loss = torch.nn.MSELoss(reduction='sum')
...
# 训练集和测试集划分
train_gt, test_gt = utils.sample_gt(gt, 0.2, mode='random')
print("Training size: {} samples selected (over {})".format(np.count_nonzero(train_gt), np.count_nonzero(gt)))

# 训练dataloader
train_dataset = datasets.HyperX_choose(img, train_gt, patch_size=3, choose_labels=[1],
                                       center_pixel=True, supervision='full')
train_dataloader = DataLoader(train_dataset, batch_size=9, drop_last=True)
...
for image,target in train_dataloader:
    print(image.shape)
    i1,i2 = torch.chunk(input=image,chunks=2,dim=2)
    z = Variable(FloatTensor(np.random.normal(0, 1, (9, opt.latent_dim))))
    # gen_img = generator(z)
    #
    # print(gen_img.shape)
    print(i1.shape)
